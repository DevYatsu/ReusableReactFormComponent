...
const InputGeneratedByForm = ({ type, name, placeholder, register, minLength, maxLength, pattern, min, max, disabled, required, getValues, isPasswordConfirm, className, label, }) => {
    const regPattern = pattern
        ? typeof pattern === "string"
            ? new RegExp(pattern)
            : { value: new RegExp(pattern.value), message: pattern.message }
        : undefined;
    return ((0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsx)("label", { htmlFor: name, className: "block text-xs font-semibold text-gray-600 uppercase dark:text-gray-200", children: label
                    ? label
                    : name
                        .split("")
...
const Input = ({ type, name, placeholder, minLength, maxLength, min, max, required, disabled, isPasswordConfirm, pattern, className, label, }) => {
    const options = {
        minLength,
        maxLength,
        min,
    ...
    console.log(options);
    return ((0, jsx_runtime_1.jsxs)("div", { className: "flex flex-col", children: [(0, jsx_runtime_1.jsx)("label", { htmlFor: name, className: "block text-xs font-semibold text-gray-600 uppercase dark:text-gray-200", children: name
                    .split("")
                    .map((letter) => letter === letter.toUpperCase() ? ` ${letter}` : letter)
